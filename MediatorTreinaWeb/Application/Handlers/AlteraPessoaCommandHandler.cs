...
using MediatorTreinaWeb.Application.Notifications;
...
namespace MediatorTreinaWeb.Application.Commands
{
    public class AlteraPessoaCommandHandler : IRequestHandler<AlteraPessoaCommand, string>
    {
        ...
        public async Task<string> Handle(AlteraPessoaCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                await _mediator.Publish(new PessoaAlteradaNotification { Id = pessoa.Id, Nome = pessoa.Nome, Idade = pessoa.Id, Sexo = pessoa.Sexo, IsEfetivado = false });
